...
export const Settings: React.FC = () => {
  ...
  const handleSelectKey = async () => {
    // Calling the environment's key selector dialog
    try {
      // @ts-ignore
        ...
        <div className="divide-y divide-slate-100">
          {items.map(item => (
            <div key={item.id} className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between hover:bg-slate-50/50 transition-colors gap-4">
               ...
               <button onClick={() => deleteEntity(item.id)} className="p-3 text-slate-400 hover:text-rose-500 rounded-xl transition-all"><Trash2 className="w-4 h-4" /></button>
            </div>
          ))}
        </div>
      </div>
      ...
      {activeTab === 'general' && (
        <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm space-y-6">
          <div className="flex justify-between items-center mb-4">
              <h3 className="font-black text-slate-800 uppercase tracking-widest text-xs">API Configuration</h3>
              <button 
              ...
              >
                  <Key className="w-3.5 h-3.5 mr-2" /> Select Gemini 3 Pro Key
              </button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 sm:p-10 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden text-center">
          {aiImportStatus === 'PROCESSING' && (
              <div className="absolute inset-0 bg-white/90 backdrop-blur-md z-10 flex flex-col items-center justify-center animate-in fade-in">
                  ...
                  <h3 className="text-lg font-black text-slate-800 uppercase tracking-widest">Pro Extraction in Progress...</h3>
                  <p className="text-slate-400 text-xs mt-2">Thinking through the document structure</p>
              </div>
          )}
          ...
          <div className="max-w-xl mx-auto space-y-8">
            <div className="flex flex-col items-center">
                ...
                <h3 className="text-xl font-black text-slate-800 uppercase tracking-widest">Gemini 3 Pro Document Scanner</h3>
            ...
            {aiImportErrorMessage && (
                <div className="bg-rose-50 border border-rose-100 p-4 rounded-2xl flex flex-col items-center gap-3 animate-in shake">
                    ...
                    >
                        <Key className="w-3.5 h-3.5 mr-2" /> Re-select Paid API Key
                    </button>
                </div>
            )}
