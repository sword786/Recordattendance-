               ...
               {editingId === item.id ? (
                   ...
                   <>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => deleteEntity(item.id)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-all"><Trash2 className="w-4 h-4" /></button>
                    </div>
                   </>
               )}
            </div>
                ...
                <div className="max-h-96 overflow-y-auto divide-y divide-slate-100">
                    {currentClassStudents.map(student => (
                        <div key={student.id} className="p-3 px-5 flex justify-between items-center hover:bg-slate-50">
                            ...
                            <button onClick={() => deleteStudent(student.id)} className="p-2 text-slate-300 hover:text-rose-500 rounded-lg transition-colors">
                      ...
                      {localTimeSlots.map((slot, idx) => (
                          <div key={idx} className="grid grid-cols-12 gap-4 items-center bg-slate-50 p-3 rounded-xl border border-slate-100">
                              ...
                              <div className="col-span-2 flex justify-center">
                                  {isEditingSlots && (
                                      <button onClick={() => handleRemoveSlot(idx)} className="p-2 text-rose-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg">
                                          <Trash2 className="w-4 h-4" />
                                      </button>
                                  )}
                              </div>
        ...
        <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm space-y-6">
          ...
          <div className="pt-8 border-t border-slate-100">
              <div className="p-4 bg-rose-50 border border-rose-100 rounded-xl flex items-center justify-between">
                  ...
                  <button onClick={() => { if(confirm('Are you absolutely sure? This cannot be undone.')) resetData(); }} className="px-4 py-2 bg-white border border-rose-200 text-rose-600 rounded-lg text-xs font-black uppercase tracking-widest hover:bg-rose-100 transition-colors">
